...
const GovSchemePage = () => {
  ...
  return (
    <Layout>
        <div className="min-h-screen bg-[#0a3330] text-white p-6">
      {/* Display detailed information of selected scheme */}
      {selectedScheme && (
        <div className="mb-12">
          <div className="p-6 border border-[#00a99d] rounded-lg shadow-md bg-[#0a2540] w-full mx-auto">
            <h2 className="text-3xl font-bold text-[#00a99d] mb-4">{selectedScheme.name}</h2>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {schemes.map((scheme, index) => (
          <div key={index} className="p-6 border border-[#00a99d] rounded-lg shadow-md bg-[#0a2540]">
            <h3 className="text-2xl font-semibold text-[#00a99d] mb-2">{scheme.name}</h3>
            <p className="text-lg text-gray-300 mb-4">{scheme.description.substring(0, 100)}...</p>
            <button
              onClick={() => handleExploreMore(scheme)}
              className="bg-[#00a99d] text-white py-2 px-4 rounded hover:bg-[#008f87] transition duration-300"
            >
